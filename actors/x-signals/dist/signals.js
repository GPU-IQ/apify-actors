...
export function detectEngagementSpike(post, avgLikes, avgReposts) {
    const SPIKE_MULTIPLIER = 3;
    return (post.likes > avgLikes * SPIKE_MULTIPLIER ||
        post.reposts > avgReposts * SPIKE_MULTIPLIER);
}
export function classifyHandle(posts, profile, followerDelta, previousAvgLikes, previousAvgReposts) {
...
export function averageEngagement(posts) {
    if (posts.length === 0)
        return { avgLikes: 0, avgReposts: 0 };
    const avgLikes = posts.reduce((s, p) => s + p.likes, 0) / posts.length;
    const avgReposts = posts.reduce((s, p) => s + p.reposts, 0) / posts.length;
