import type { IgPost, IgProfile, SignalType } from './types.js';
...
export function classifyPost(post: IgPost): SignalType[] {
...
export function classifyHandle(
  ...
  if (followerDelta !== null && profile.followerCount > 0) {
    ...
    if (pct >= 0.05) signals.add('follower_growth');
  }
  ...
  if (posts.length > 0 && avgLikesBaseline !== null) {
    ...
    if (currentAvg > avgLikesBaseline * 3) signals.add('engagement_spike');
  }

  return [...signals];
}

export function averageEngagement(posts: IgPost[]): number {
