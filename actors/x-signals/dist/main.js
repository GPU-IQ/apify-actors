...
const { handles = [], keywords = [], mode = 'both', maxPostsPerHandle = 25, maxSearchResults = 50, followerBaselinePath = 'follower-baseline', } = input;
...
const crawler = new PlaywrightCrawler({
    ...
    async requestHandler({ page, request, log }) {
        const { type, handle, keyword } = request.userData;
        ...
        if (type === 'keyword' && keyword) {
            ...
            try {
                ...
                log.error(`Failed keyword search "${keyword}": ${err.message}`);
                await Actor.pushData({
                    keyword,
                    posts: [],
                    scrapedAt: new Date().toISOString(),
