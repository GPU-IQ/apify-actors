...
export function parseProfileFromApi(data) {
    try {
...
export function parsePostsFromApi(data, handle) {
    try {
        // GraphQL shape (web)
        ...
        return items.map((item) => {
            ...
            const mediaType = item.__typename === 'GraphVideo' || item.media_type === 2
                ? 'video'
...
export async function scrapeHashtag(page, hashtag, maxResults) {
    const results = [];
    const onResponse = async (response) => {
        const url = response.url();
        if (!url.includes('hashtag') && !url.includes('tags'))
            return;
        try {
            const json = await response.json();
            ...
            for (const edge of edges) {
                ...
                if (post)
                    results.push({ ...post, authorHandle });
            }
        }
        catch {
            // skip
